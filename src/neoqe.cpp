 ...
 */
NeoQE::NeoQE(BoltValue params)
{
    // test if connection parameters contains any "routing" fields, if so
    //  set routing mode and build the routing table

    // first make sure everything in the key section is lower case
    //for (size_t i = 0; i < params.map_val.size; i++) 
    //{
    //    BoltValue v = *params.pool->Get(params.map_val.key_offset);
    //    String_ToLower((char*)v.str_val.str, v.str_val.length);
    //} // end for

    //// do we have a routing key?
    //cluster_mode = false;
    //if (params["routing"].type == BoltType::Unk)
    //    cluster_mode = false;
...
// */
//inline bool NeoQE::Flush_And_Poll(BoltValue& v)
//{
//    BoltMessage reset(v);
